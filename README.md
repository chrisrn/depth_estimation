...
The default model we use is the [unetplusplus](https://github.com/chrisrn/depth_estimation/blob/master/src/UNet_plus_plus.py) and we use the default value of resnet18 with almost 16M parameters but for the experiments on CPU we used the lightest model which is a UNet based on mobilent v2 with 6.8M parameters. It's important also to mention that we can choose the pre-trained weights of the encoder and in this case we use the default weights from imagenet. 
...
### Results of UNet
Below we can see some result depth maps (image/target/prediction) after training a UNet with mobilenet v2 encoder-decoder and the encoder pre-trained on imagenet. After a long training of only 3 epochs on CPU the model achieved an SSIM=0.88. If we use a deeper encoder-decoder model (like resnet 18) and train for more epochs on GPU the results will be promising.
